import { existsSync, Logger } from './deps.ts';
...
export const createFolderOrFileHandler = async (req: Request) => {
  ...
  const metadataFileContentWithoutValues = config.allowedKeys.reduce(
    (a: { [key: string]: string }, b: string) => (a[b] = '', a),
    {},
  );
  try {
    ...
    Deno.writeTextFile(
      ...
      JSON.stringify(metadataFileContentWithoutValues, null, 2),
    );
...
export const getEditorUIHandler = async (req: Request) => {
  ...
  const newContent = contents.replace(
    '<!-- INJECTION -->',
    `<script src="injectedData.js?artefact=${artefact}&image=${image}"></script>`,
  );
...
export const updateMetadataHandler = async (req: Request) => {
  ...
  const requestPath = path.split('/');
  const filePath = buildFilePath(requestPath[1], requestPath[2]);
  const metadata = await getMetadata(filePath).catch((e) => {
    console.info(e);
  });
...
const buildFilePath = (artefact: string, image: string) =>
  getFilePathInformation(artefact, image).filePath;
